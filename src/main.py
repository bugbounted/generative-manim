...
if st.checkbox("از کلید توسعه دهنده اوپن ای آی خود استفاده کنید (توصیه می شود)"):
  openai_api_key = st.text_input(
      "[کلید توسعه دهنده اوپن ای آی]خودتان را بچسبانید (https://platform.openai.com/account/api-keys)", value="", type="password")
...
if generate_video:
  ...
  try:
    ...
    if openai_model.lower() == "gpt-4":
      st.error(
          "Error: در حال حاضر اوپن ای آی هر سه ساعت بیست و پنج درخواست برای جیپیتی فور می پذیرد. این یعنی اوپن آی آی شروع به رد کردن برخی از درخواست ها خواهد کرد. دو راه حل وجود دارد: از جی پی تی ۳.۵  استفاده کنید یا از کلید توسعه دهنده اوپن ای آی شخصیتان استفاده کنید.")
      st.stop()
    else:
      st.error(
